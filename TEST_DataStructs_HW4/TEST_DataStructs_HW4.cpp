...
TEST_CLASS(TEST_Insert){public :

                            // add to beginning
                            TEST_METHOD(beginning){HashOneD t;
t.Insert(0);
...
// add same values multiple times, and add a value that would be in a spot thats
// overflowed
...
TEST_CLASS(TEST_Find){public :

                          // add integer, find it
                          TEST_METHOD(one_int){HashOneD t;
t.Insert(375);
...
// find - insert thing that loops over, remove all that are at end, find thing
// that wrapped around
TEST_METHOD(wrap_around) {
  HashOneD t;
...
TEST_CLASS(TEST_Remove){// insert val, remove that val
                        TEST_METHOD(insert_remove){HashOneD t;
t.Insert(547);
...
TEST_CLASS(TEST_Insert){
    ...
    // kinda included a bunch of other tests in here. this is all we need pretty
    // much, right?
    TEST_METHOD(full_col){HashTwoD t;
...
TEST_CLASS(TEST_Find){
    ...
    // find first val
    TEST_METHOD(find_val){HashTwoD t;
t.Insert(432);
...
TEST_CLASS(TEST_Remove){// insert then remove
                        TEST_METHOD(insert_remove){HashTwoD t;
t.Insert(543);
