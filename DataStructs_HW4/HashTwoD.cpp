...
int HashTwoD::Hash(int inval) { return inval % SIZE_2D; }
...
process
        ...
        otherwise loop through column to insert (incrementing colls in the
process) output # collisions
...
int HashTwoD::Remove(int inval) {
  ...
  // find the value
  // loop through the column until encounter it, encounter a min, or curr depth
  // >= DEPTH
  int curr_depth{0};
  while (table[index][curr_depth] != inval &&
         table[index][curr_depth] != std::numeric_limits<int>::min() &&
...
int HashTwoD::GetVal(int index, int depth) { return table[index][depth]; }
