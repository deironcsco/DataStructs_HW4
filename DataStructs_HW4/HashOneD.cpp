...
/*
constructor
initializes all values in table to 0, and all bools in is_empty_f to true (meaning they are empty)
*/
HashOneD::HashOneD() {
...
bool HashOneD::IsFull() {
	for (int i{ 0 }; i < SIZE; i++) {
		if (is_empty_f[i]) { // returns false if at least one of the entries is empty
			return false;
		}
	}
	return true;
...
/*
Insert – accepts a value (integer), runs a hash function on the integer, and places it
...
process
	uses Hash() to find wehre to put integer, if collision it goes to the next index until its empty...
...
/*
Find – accepts a value (integer), locates the value in the hash table and returns the
...
int HashOneD::Remove(int inval) {
	...
	if (table[index] == inval) {
		table[index] = std::numeric_limits<int>::min(); //some val that's pretty unlikely
	}
	//does not change is_empty_f because dupe values could still be after removed value
...
/*
Print – prints all items in the hash table including an indication of which spots are
not occupied
*/
void HashOneD::Print() {
	for (int i{ 0 }; i < SIZE; i++) {
...
//get table[inval]
int HashOneD::GetVal(int inval) {
	return table[inval];
}

//get is_empty_f[inval]
bool HashOneD::GetFlag(int inval) {
	return is_empty_f[inval];
}